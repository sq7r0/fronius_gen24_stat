...
class FroniusToInflux:
    ...
    def __init__(self, client: InfluxDBClient, location: LocationInfo, endpoints: List[str], tz: Any) -> None:
#    def __init__(self, client: InfluxDBClient, endpoints: List[str], tz: Any) -> None:
...
#                        'YEAR_ENERGY': self.get_float_or_zero('YEAR_ENERGY'),
        ...
        elif collection == 'MinMaxInverterData1':
            return [
                {
                    'measurement': collection,
                    'time': timestamp,
    ...
    def sun_is_shining(self) -> None:
        sunlocal = sun(self.location.observer,(datetime.datetime.now(tz=self.tz)))
        if not self.IGNORE_SUN_DOWN and not sunlocal['sunrise'] < datetime.datetime.now(tz=self.tz) < sunlocal['sunset']:
            raise SunIsDown
        return None
